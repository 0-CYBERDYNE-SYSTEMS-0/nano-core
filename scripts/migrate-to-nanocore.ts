...
const SourceTypeSchema = z.enum(["openclaw", "clawdbot", "moltbot", "hermes", "auto"]);
...
interface MigrationItem {
  ...
  sourcePath: string;
  targetPath: string;
  status: "migrated" | "skipped" | "conflict" | "error" | "would_migrate" | "archived";
  reason?: string;
  backupPath?: string;
}
...
const SOURCE_PATHS: Record<Exclude<SourceType, "auto">, { dir: string; config: string }> = {
...
async function parseOpenClawConfig(configPath: string): Promise<SourceConfig> {
  ...
  return {
    ...
    heartbeatInterval: data.heartbeat?.enabled ? data.heartbeat?.interval : undefined,
...
class Migrator {
  ...
  constructor(
    ...
    this.reportDir = args.outputDir || path.join(repoRoot, "migration", source.type, timestamp);
  ...
  private async backupIfExists(targetPath: string): Promise<string | undefined> {
    ...
    if (targetPath.startsWith(this.targetWorkspace)) {
      backupPath = path.join(this.backupDir, path.relative(this.targetWorkspace, targetPath));
    } else {
  ...
  private addItem(item: Omit<MigrationItem, "status"> & { status?: MigrationItem["status"] }): void {
    this.items.push({
      status: "migrated",
      ...item,
    } as MigrationItem);
...
`;
  ...
  private async migrateMemory(): Promise<void> {
    ...
    if (existsSync(targetMemoryPath) && !this.args.overwrite) {
      ...
      memoryContent = existingContent + "\n\n<!-- Merged from " + this.source.type + " -->\n" + newLines.join("\n");
    }
...
`;
  ...
  private async migratePrinciples(): Promise<void> {
    ...
    const targetPrinciplesPath = path.join(this.targetWorkspace, "PRINCIPLES.md");
...
`;
  ...
  private async updateEnvFile(envVars: Record<string, string>, category: string): Promise<void> {
    let existingContent = "";
    ...
    const backupPath = this.args.overwrite ? await this.backupIfExists(this.targetEnv) : undefined;
    ...
    this.addItem({
      ...
      targetPath: this.targetEnv,
      status: this.args.dryRun ? "would_migrate" : "migrated",
      reason: this.args.dryRun ? `Would update .env with ${category} settings` : undefined,
      backupPath,
    });
  }
  ...
  private async migrateSkills(): Promise<void> {
    ...
    const targetSkillsDir = path.join(this.targetWorkspace, "skills", `${this.source.type}-imports`);
    ...
    for (const skillDir of skillDirs) {
      ...
      if (existsSync(targetSkillPath)) {
        switch (this.args.skillConflict) {
          case "skip":
            ...
            continue;
          case "rename":
            targetSkillPath = path.join(targetSkillsDir, `${skillDir.name}-imported`);
            break;
          case "overwrite":
      ...
      this.addItem({
        ...
        targetPath: targetSkillPath,
        status: this.args.dryRun ? "would_migrate" : "migrated",
        reason: this.args.dryRun ? `Would copy skill ${skillDir.name}` : undefined,
        backupPath,
      });
      migratedCount++;
    }
...
`;
  ...
  private async migrateAllowlist(): Promise<void> {
    if (!this.config.execPatterns || this.config.execPatterns.length === 0) {
      this.addItem({
        ...
        category: "allowlist",
        sourcePath: this.source.configPath,
        targetPath: path.join(os.homedir(), ".config/fft_nano/mount-allowlist.json"),
        status: "skipped",
    ...
    const allowlistPath = path.join(os.homedir(), ".config/fft_nano/mount-allowlist.json");
    ...
    const newPatterns = this.config.execPatterns.filter((p) => !existingPatterns.includes(p));
    ...
    if (!this.args.dryRun) {
      ...
      await fs.writeFile(allowlistPath, JSON.stringify({ patterns: mergedPatterns }, null, 2), "utf-8");
    }
  ...
  private async migrateAgentConfig(): Promise<void> {
    ...
    if (this.config.memoryFlushEnabled !== undefined) {
      const parityPath = path.join(os.homedir(), ".config/fft_nano/runtime.parity.json");
      ...
      if (!this.args.dryRun) {
        ...
        await fs.writeFile(parityPath, JSON.stringify(parityConfig, null, 2), "utf-8");
      }
  ...
  private async migrateArchive(): Promise<void> {
    ...
    if (this.config.multiAgentEnabled !== undefined) {
      archiveItems.push({ name: "multi-agent", data: { enabled: this.config.multiAgentEnabled } });
    }
    ...
    for (const item of archiveItems) {
      ...
      if (!this.args.dryRun) {
        await fs.writeFile(archivePath, JSON.stringify(item.data, null, 2), "utf-8");
      }
      ...
      this.addItem({
        ...
        reason: this.args.dryRun ? `Would archive ${item.name}` : `Archived ${item.name} for manual review`,
      });
    }
  ...
  private generateReport(): MigrationReport {
    const summary = {
      migrated: this.items.filter((i) => i.status === "migrated" || i.status === "would_migrate").length,
  ...
  private generateSummaryMarkdown(report: MigrationReport): string {
    ...
    // Migrated items
    const migrated = report.items.filter((i) => i.status === "migrated" || i.status === "would_migrate");
  ...
  private generateMigrationNotes(report: MigrationReport): string {
    ...
    sections.push("");
    sections.push(`This file contains notes and action items from the migration from ${report.sourceType}.`);
    sections.push("");
    ...
    const needsReview = report.items.filter(
      (i) => i.status === "archived" || i.status === "conflict" || i.status === "skipped"
    );
    ...
    if (needsReview.length > 0) {
      ...
      sections.push("All items migrated successfully. No manual review required.");
      sections.push("");
    }
...
async function main(): Promise<void> {
  ...
  if (args.source === "auto") {
    ...
    const migrator = new Migrator(args, source, config, targetWorkspace, targetEnv, repoRoot);
