...
function inferTimeOfDay(now: Date): DeviceContext['timeOfDay'] {
...
function inferSeason(now: Date): DeviceContext['season'] {
...
function deriveContext(
  ...
  const alertLevel: DeviceContext['alertLevel'] = hasCritical
    ? 'critical'
    : hasWarning
      ? 'warning'
      : 'normal';

  let suggestedTheme: DeviceContext['suggestedTheme'];
  if (
...
export function stopStateCollector(): void {
  if (!running) return;
