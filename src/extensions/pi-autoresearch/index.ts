...
function appendRightAlignedAdaptiveHint(
  ...
  for (const candidate of candidates) {
    ...
    if (leftWidth + hintWidth <= width) {
      return (
        left + ' '.repeat(Math.max(0, width - leftWidth - hintWidth)) + hint
      );
    }
    ...
    return (
      truncatedLeft +
      ' '.repeat(Math.max(0, width - truncatedLeftWidth - hintWidth)) +
      hint
    );
  }
...
function renderDashboardLines(
  ...
  const primaryW = Math.max(
    11,
    Math.min(Math.floor(width * 0.25), visibleWidth(primaryLabel) + 1),
  );
  ...
  while (
    ...
    totalSecWidth() > width - fixedW - minDescW
  ) {
  ...
  const baselineIndex = st.results.findIndex(
    (x) => x.segment === st.currentSegment,
  );
...
export default function autoresearchExtension(pi: ExtensionAPI) {
  ...
  const sendPendingResumeIfReady = (
    ctx: ExtensionContext,
    runtime: AutoresearchRuntime,
  ): void => {
  ...
  const hasRunExperimentsThisSession = (
    runtime: AutoresearchRuntime,
  ): boolean => runtime.experimentsThisSession > 0;
    ...
    if (runtime.dashboardExpanded) {
      ...
      ctx.ui.setWidget('autoresearch', (tui, theme) => ({
        render(width: number): string[] {
          ...
          const fillLen = Math.max(
            0,
            safeWidth - 3 - 1 - visibleWidth(title) - 1,
          );
      ...
      ctx.ui.setWidget('autoresearch', (tui, theme) => ({
        render(width: number): string[] {
          ...
          if (checksFailed > 0)
            optional.push(theme.fg('error', ` ${checksFailed}⚠`));
  ...
  function writeDashboardFile(workDir: string): string {
    ...
    const exportDir = fs.mkdtempSync(
      path.join(tmpdir(), 'pi-autoresearch-dashboard-'),
    );
  ...
  async function exportDashboard(ctx: ExtensionContext): Promise<void> {
    ...
    if (!fs.existsSync(jsonlPath)) {
      ctx.ui.notify(
        'No autoresearch.jsonl found \u2014 run some experiments first',
        'error',
      );
      return;
    }
  ...
  pi.registerCommand('autoresearch', {
    ...
    handler: async (args, ctx) => {
      ...
      if (command === 'off') {
        ...
        ctx.ui.notify(
          wasRunning
            ? 'Autoresearch mode OFF — aborting current run'
